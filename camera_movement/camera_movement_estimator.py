...
class CameraMovementEstimator():
    def __init__(self, frame):
        first_frame_gray_scale  = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        mask_features = np.zeros_like(first_frame_gray_scale)
        ...
        for object, object_tracks in tracks.items():
            for frame_num, track, in enumerate(object_tracks):
                for track_id, track_info in track.items():
                    ...
                    true_position = (position[0] - camera_movement[0], position[1] - camera_movement[1])
                    tracks[object][frame_num][track_id]['true_position'] = true_position
